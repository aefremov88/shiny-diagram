...
- **`Gesture targets:`** — an optional list of local rendered targets on which user gestures can act. Each entry contains a local Address and the declarative Query that identifies the rendered target. The section describes only structure owned by this UI component. It never describes where the component is composed in View.
